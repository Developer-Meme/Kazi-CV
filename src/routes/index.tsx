...
import { defaultResume, TEMPLATES, uid, blankItem, type ResumeSection, type ResumeState, type SectionKind, type TemplateId } from "@/lib/resume-types";
...
export const Route = createFileRoute("/")(({
  head: () => ({
    meta: [
      { title: "KaziCV — Professional Resume Builder" },
      { name: "description", content: "Build a polished single-page resume. Drag to reorder sections, swap templates instantly, and export clean HTML." },
      { property: "og:title", content: "KaziCV — Professional Resume Builder" },
      { property: "og:description", content: "Drag-and-drop resume builder with live templates and HTML export." },
    ],
  }),
  component: Index,
}));
...
function Index() {
  ...
  const [state, setState] = useState<ResumeState>(() => defaultResume());
  ...
  const addSection = (kind: SectionKind) => {
    ...
    setState((s) => ({
      ...
      sections: [
        ...s.sections,
        { id: uid(), kind, title: titles[kind], items: [blankItem(kind)] },
      ],
    }));
  };
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <div className="max-w-[1600px] mx-auto grid grid-cols-1 lg:grid-cols-[380px_1fr] gap-6 p-6">
        ...
        <aside className="space-y-4">
          ...
          >
            ...
            <Input value={state.profile.name} onChange={(e) => setState((s) => ({ ...s, profile: { ...s.profile, name: e.target.value } }))} placeholder="Full name" />
            <Input value={state.profile.title} onChange={(e) => setState((s) => ({ ...s, profile: { ...s.profile, title: e.target.value } }))} placeholder="Title" />
            <div className="grid grid-cols-2 gap-2">
              <Input value={state.profile.email} onChange={(e) => setState((s) => ({ ...s, profile: { ...s.profile, email: e.target.value } }))} placeholder="Email" />
              <Input value={state.profile.location} onChange={(e) => setState((s) => ({ ...s, profile: { ...s.profile, location: e.target.value } }))} placeholder="Location" />
            </div>
            <Textarea value={state.profile.summary} onChange={(e) => setState((s) => ({ ...s, profile: { ...s.profile, summary: e.target.value } }))} placeholder="Short summary" rows={3} />
          </motion.div>
      ...
      <footer className="mt-8 border-t border-border/40 py-6 text-center">
        <p className="text-xs text-muted-foreground">
          KaziCV — Built for Kenya's professionals
        </p>
      </footer>
    </div>
  );
